...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> loadData() async {
    ...
    try {
      final categoriesData = ApiService.getCategories();
      final dishesData = await ApiService.getMenuItems(category: selectedCategoryId?.toString());
      
      setState(() {
        categories = categoriesData.map((name) => Category(
          id: categoriesData.indexOf(name) + 1,
          name: name,
  ...
  Future<void> filterByCategory(int? categoryId) async {
    ...
    try {
      ...
      if (categoryId != null && categoryId > 0) {
        final categoryList = ApiService.getCategories();
        if (categoryId <= categoryList.length) {
          categoryName = categoryList[categoryId - 1];
        }
      }
      
      final dishesData = await ApiService.getMenuItems(category: categoryName);
      setState(() {
        dishes = dishesData;
        isLoading = false;
      });
